...
② 빈 양식은 양식 ID로 식별하며, 파일명·페이지수·AcroForm 여부·저장경로·업로드일시 정보를 포함한다.
...
⑥ 하나의 지식 문서는 여러 청크로 분할되며, 청크는 청크 ID로 식별하고 본문·순번·글자수 정보를 포함한다. (임베딩 벡터는 ChromaDB에 별도 저장)
...
|---|---|
| 사용자 | ID, 이메일, 이름, 생성일시 |
| 빈 양식 | 양식 ID, 사용자 ID, 파일명, 페이지수, AcroForm 여부, 저장경로, 업로드일시 |
| 폼필드 | 필드 ID, 양식 ID, 필드명, 라벨, 타입, 페이지, 좌표(x0·y0·x1·y1), 필수여부, 최대길이 |
...
| KPI 평가 | 평가 ID, 데이터셋, 정확도, 시간단축, 근거율, 외부호출수, 측정일시 |
